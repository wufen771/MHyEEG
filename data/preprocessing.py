import logging
logging.basicConfig(level=logging.DEBUG)  # 设置日志记录级别为 DEBUG
import argparse
import os
...
def preprocess(sessions_dir, save_path, verbose=False):
    physio_file_failed_list = []
    gaze_file_failed_list = []
    for dir_id in tqdm(range(len(sessions_dir)), desc='Preprocessing'):
        ...
        feltArsl = root.attrib.get('feltArsl', "0")
        feltVlnc = root.attrib.get('feltVlnc', "0")
    #     if 'feltEmo' in root.attrib:
    #         feltEmo = root.attrib['feltEmo']
    # # 使用 feltEmo 进行进一步处理
    #     else:
    #         pass
        ...
        try:
            ...
            try:
                ...
                if not video_start: # empty list 
                    ...
                    print("Video start index:", video_start)
                    # need to get data right after video started
                ...
                print("Video end index:", video_end)
                ...
                print("gaze_file No such this file: {}".format(gaze_file))
                gaze_file_failed_list.append(gaze_file)
                pass
        except:
            print("physio_file No such this file: {}".format(physio_file))
            physio_file_failed_list.append(physio_file)
            pass
    print("physio_file failed list: {}".format(physio_file_failed_list))
    print("gaze_file failed list: {}".format(gaze_file_failed_list))
        

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--sessions_path', type=str, default='C:\\Users\\ff\\Desktop\\hci-tagging-database\\Sessions', help='Path to Sessions folder')
...
# try:
#     gaze_file = "C:\\Users\\ff\\Desktop\\code\\MHyEEG\hci-tagging-database\\preproc_data"  # 将文件名替换为实际文件名
#     gaze_df = pd.read_csv(gaze_file, sep='\t', skiprows=23)
# except pd.errors.ParserError as e:
#     print("An error occurred while parsing the CSV file:", e)
